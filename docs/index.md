...
- [docs/adr/](adr/) — decisions and their rationale. Append-only.
  ...
  - [ADR-0013](adr/0013-the-environment-names-the-venv.md) — the environment names the venv: `UV_PROJECT_ENVIRONMENT=.venv-sandbox` in every sandbox container, `.venv` on every host; a repo never chooses *(Accepted 09-15; shared number with macolima)*
  - ADR-0014 — **reserved**: the number is taken by the channel-side ADR (`vendor-tools.sh` cites it) and by macolima's pending paperbridge-delivery decision; not reused here
  - [ADR-0015](adr/0015-the-sandbox-briefs-agents-from-their-homes.md) — the sandbox briefs agents from their global homes (`~/.claude/CLAUDE.md`, `~/.gemini/config/rules/sandbox-notice.md`), one neutral text, never a block in a repo *(Accepted 09-15; shared number with macolima)*
- [work/](../work/) — in-flight items AND new proposals, archived on merge ([README](../work/README.md), [ADR-0010](adr/0010-one-proposal-home-close-the-rfc-tier.md))
  ...
  - [0030-venv-and-notice-replay](../work/0030-venv-and-notice-replay/spec.md) — the two ferried replays from macolima (0008 venv rule, 0011 notice homes) run on this machine: ADR-0013 + ADR-0015, the strip, the scan, the profile recreates; **R1 built**, R2–R6 per machine
...
| — | [`scripts/agent-notice.test.sh`](../scripts/agent-notice.test.sh) | sandbox-notice content suite (13, offline) — locks the two defects only an agent home can see: no repo-relative path, no host-side mechanism. The notice is read from `~/.claude/CLAUDE.md` and `~/.gemini/config/rules/`, filesystems where this repo does not exist |
| — | [`scripts/sync-agent-notice.test.sh`](../scripts/sync-agent-notice.test.sh) | notice sync suite (72, offline) — every BEGIN spelling is one slot (neutral, both legacy, a four-line hand-written one), text outside the region is byte-identical, `--strip` eats the blank after END and never creates a file, update and strip keep the target's mode ([ADR-0015](adr/0015-the-sandbox-briefs-agents-from-their-homes.md)) |
| — | [`scripts/workspace-scan.test.sh`](../scripts/workspace-scan.test.sh) | workspace-scan suite (49, offline; throwaway git repos in a temp dir) — venv ownership by shebang, the `.venv*/` ignore trap, ask-fence spellings, `NOTICE-IN-REPO`, `--out` refusing a tracked path ([ADR-0013](adr/0013-the-environment-names-the-venv.md), [ADR-0015](adr/0015-the-sandbox-briefs-agents-from-their-homes.md)) |
...
- [wsl_insert-guide.md](../host_setup/wsl_insert-guide.md) — /etc/wsl.conf insert settings per mode: `ro` (C: read-only) and `isolated` (no /mnt/c, no interop)
...
- [`scripts/sync-agent-notice.sh`](../scripts/sync-agent-notice.sh) — write/refresh the managed sandbox-notice block into the two agent homes per profile (`claude-home/CLAUDE.md`, `gemini-home/config/rules/sandbox-notice.md`; source: `sandbox_templates/common/agent-notice.md`), run by `profile.sh` on every `up`/`recreate`/`rebuild`/`converge`. Never into a repo ([ADR-0015](adr/0015-the-sandbox-briefs-agents-from-their-homes.md)); `--strip` retires a block that was placed in one by hand
- [`scripts/workspace-scan.py`](../scripts/workspace-scan.py) — host-side, read-only, stdlib-only scan of every git repo in every profile's workspace (`~/repo/<p>/`) against ADR-0013/0015: AGENTS.md source + CLAUDE.md stub, no notice block in a repo, settings hygiene and ask-fence spellings, venv ownership by shebang and slot, `.local` ignores. `just workspace-scan`; the report names every repo, so `--out` refuses any path git would track
