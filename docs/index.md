...
| — | [`scripts/antigravity-parity.test.sh`](../scripts/antigravity-parity.test.sh) | `agy` policy suite (28, offline — no docker, no `agy`) — diffs the two static deny lists exactly in both directions, locks that the two hook failure postures stay OPPOSITE, and that convergence merges rather than mirroring ([ADR-0006](adr/0006-antigravity-is-two-layer-like-claude.md)) |
