...
- [`scripts/sync-agent-notice.sh`](../scripts/sync-agent-notice.sh) — write/refresh the managed sandbox-notice block into Claude Code's home per profile (`claude-home/CLAUDE.md`; source: `sandbox_templates/common/agent-notice.md`), run by `profile.sh` on every `up`/`recreate`/`rebuild`/`converge`. Never into a repo ([ADR-0015](adr/0015-the-sandbox-briefs-agents-from-their-homes.md)); `--strip` retires a block that was placed in one by hand
