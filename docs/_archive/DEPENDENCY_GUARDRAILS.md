...
- Status: **Accepted → shipped** in phase 0 (T02, 2026-07-31) via
...
> **This is now enforced — do not edit this file to change agent behaviour.**
> §2's rules live in `sandbox_templates/common/agent-notice.md`, which
> `scripts/sync-agent-notice.sh` propagates into repo `AGENTS.md` files. That notice is the
> operative text; this document is the rationale behind it and will drift.
>
> Enforcement is no longer prose alone. As of phase 1, `deny-destructive.sh` rule 13
> **blocks** an edit that introduces a new dependency name to a manifest (a version bump
> passes — that distinction is the merge gate), and rule 14 **warns** on install commands
> added to instruction files. `claude-settings.json` denies 87 install invocations.
