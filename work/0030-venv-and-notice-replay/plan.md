...
| B8 | Prove the rule in one shell inside a running container, variable prefixed per command, `--offline`, against a real checkout; host `.venv` byte-identical after | owner at the prompt | **done 2026-09-15** (mlpipe) |
| merge | Merge R1. From here any profile's next recreate switches it | owner | **done** `8c3ed16` |
| R4a | **Recreate every profile in one sitting**; `just verify <p>` each: `UV_PROJECT_ENVIRONMENT=.venv-sandbox` PASS, both notice lines PASS | owner | **done 2026-09-15**, all three green |
| R4b | Pull the shared repos — the pulls carry the venv edits (the Mac's strips turned out unpushed), which is why they wait for the switch | owner | **done 2026-09-15**; three therapod pulls resolved by hand (notes) |
...
| R3b | Re-run the scan on the day; `--fail-on HARDCODED-VENV-LINUX,OS-VENV-SELECT` names the repos that get a 0008 handoff (final form, citing agentic-conventions ADR-0017; precondition `echo $UV_PROJECT_ENVIRONMENT` = `.venv-sandbox`) | agent | **done 2026-09-15**: NOTICE-IN-REPO exits 0; venv gate names the two handoffs, delivered to `inbox/0030/` |
