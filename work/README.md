...
| [0022](0022-port-forward-to-macolima/spec.md) | Port this repo's work forward to macolima. Re-validates and extends `macolima@work/0001` (planned 2026-08-23, never started): re-diff done, Phase D unblocked by ADR-0007, anchor moved to `main@eda42dd`. Three stages — select/filter, implement what is implementable from here, then a handoff document for the macOS side | **Draft** — the 21-commit ComfyUI/fal branch is excluded wholesale (§5); the 10-vs-1 offline test-suite gap is the highest-leverage row; exits only on Mac-side confirmation |
