...
| [0029](0029-proxmox-comp-hardening/spec.md) | **Proxmox-comparison hardening.** A VM design review (Proxmox VE, VLAN, pfSense) found a stronger *escape* boundary than this stack and nothing at the agent-policy layer — and, turned back on this repo, the same two gap classes: allowlisted egress that doubles as an exfil channel (the fetch-on-behalf reader services, `openrouter`/`openai`/`fal` always-on), and secrets the agent can `cat` (`.credentials.json`, `hosts.yml` with gh's default `gist` scope, every `secrets.env` key). Ten findings; the one move that collapses three of them is a **credential relay sidecar** behind `ANTHROPIC_BASE_URL` | **Draft** — opened 2026-09-15; five decision gates (§4), D1 (reader services: drop vs relay) and D2 (relay scope) shape Phase C. Phase A is host-side with no code and can run today. Sibling: `macolima@work/0012` |
| [0030](0030-venv-and-notice-replay/spec.md) | **Replaying macolima's 0008 + 0011 here.** The environment names the venv ([ADR-0013](../docs/adr/0013-the-environment-names-the-venv.md): compose `UV_PROJECT_ENVIRONMENT=.venv-sandbox`, hook carve-out moves off `.venv`, `verify` asserts it, `workspace-scan.py` is the done-check) and the sandbox briefs agents from their global homes ([ADR-0015](../docs/adr/0015-the-sandbox-briefs-agents-from-their-homes.md): one neutral notice shared verbatim with the sibling, two targets per profile, `--strip` for the 21 blocks found in repos here, `NOTICE_SHA` hash into `verify`). Two ferried replay files are the spec; this item holds only what differs on a Linux host | **R1 built 2026-09-15** on `dev/0030-venv-and-notice-replay`, twelve suites green; scan run (52 repos). Next is the owner's: B8 in one shell, merge, recreate every profile in one sitting, THEN pull/strip/handoffs (plan.md) |
