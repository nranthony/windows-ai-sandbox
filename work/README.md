...
| [0020](0020-fal-media-tooling-provisioning/spec.md) | fal.ai media tooling — triage of a 43-item external checklist written without this repo's context: 17 are already answered by ADR-0003/0005/0007/0011 and the vendor+converge machinery, 4 presume a tenant fleet that does not exist, 22 are real. The five that matter: no working run has ever happened (the host list is a probe plus a source read), a paid submit can be orphaned by a mid-job denial while the evidence sits on tmpfs, `FAL_KEY` is the first key in `secrets.env` the agent can spend by itself, spend has no cap/telemetry/attribution, and nothing is installed — no client, no CLI, **no ffmpeg** | **Draft** — five decision gates (§6); D1 is one ephemeral end-to-end run that makes the other four cheaper |
