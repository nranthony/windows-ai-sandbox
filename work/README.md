...
| [0016](0016-comfyui-into-the-sandbox/spec.md) | Run ComfyUI inside `ai-sandbox-<profile>` and retire `my_comfyui/.devcontainer/`. Carries the model-download host census (527 Manager entries; 91% huggingface.co) and the reason the CDN hosts must be observed in Squid's log rather than written from source — the Xet CAS URL arrives in a response header | **All four decisions closed 2026-08-28**, **zero egress opened** (D1 = Manager `network_mode = offline`). Devcontainer deleted, ComfyUI verified on GPU (torch 2.10.0+cu130 on the 12.6 base), `libgl1` added to the image (D4). **Blocked on one owner action — image rebuild + per-profile recreate — then archivable** |
