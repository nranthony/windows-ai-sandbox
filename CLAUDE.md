...
- **GPU passthrough** (WSL2 hosts only): `docker-compose.wsl-gpu.yml` overlay — `/dev/dxg` + `/usr/lib/wsl` volume + `LD_LIBRARY_PATH=/usr/lib/wsl/lib`. NOT `--gpus all` (broken under NVIDIA Container Toolkit ≥1.18). `profile.sh` layers the overlay automatically when `/dev/dxg` exists (`SANDBOX_GPU=0|1` overrides); bare-Linux hosts run the same base compose GPU-less.
