...
│   └── antigravity-cli/settings.json         (agy's own prefs AND our permissions.allow/ask/deny —
│                                              MERGED on up, never overwritten; ADR-0006)
...
| Agent tools (`agy`) | `sandbox_templates/antigravity/antigravity-settings.json` carries the same deny set in agy's `command(...)` grammar, diffed against Claude's by `antigravity-parity.test.sh`. The **static** list is the load-bearing layer here: a workspace `.agents/hooks.json` outranks the global hook and can disable it by name (measured), while nothing in a workspace reaches `settings.json`. Same hook engine, `--dialect=antigravity`, fail-CLOSED. [ADR-0006](docs/adr/0006-antigravity-is-two-layer-like-claude.md) |
