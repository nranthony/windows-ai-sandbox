...
`sandbox_templates/skills/myclickup/` is a THIRD kind of entry: neither
sandbox-native nor from agentic-conventions, so `UPSTREAM.md` does not list it
(that file is generated by the skill sync, which would delete any row it did not
write). It is generated by `scripts/vendor-myclickup.sh` from
`packaging/sandbox/SKILL.md` in the private `nranthony/myclickup` checkout, and a
local edit here is reverted by the next vendor run — `just vendor-check` fails on
the drift first.

## Vendored wheels (private tools)

```bash
just vendor-myclickup          # build from source + copy wheel & skill into the build context
just vendor-check              # fail if the vendored payload drifted from its source
